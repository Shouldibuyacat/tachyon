...
#if defined(NDEBUG)
# error "Tachyon cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe1e56b73d357433fcd53ad1edd3ff500d8afad740e2898811af363376e8e58d8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tachyon: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tachyon Signed Message:\n";
...
{
    // Tachyon: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 500000); // Tachyon: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Tachyon: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Tachyon: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tachyon: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tachyon: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x12ba61437aaa8323fc736dd5b6a2af51606b33f66bddabf932775b10f8348f39");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CTV News 21/Jan/2018 Papua New Guinea volcano erupts in spectacular fashion";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e4c6fb115eda7759da1dc564b56b0a64a6ddbdf27a65ec6a78bd528ebdc0e676ae022bbc21f5a1a2e2bfa89b6aec0d712539e2c68a069de59f40eb15f94495f2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516567225;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 386912121;

        if (fTestNet)
        {
            block.nTime    = 1516567200;
            block.nNonce   = 385869306;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x941cd909dec49bf2aae76c71f44cabf52d440bf7197ca054f3f8d1824dee3f3f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc8, 0xb7, 0xd4 }; // Tachyon: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TachyonMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TachyonMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TachyonMiner(CWallet *pwallet)
{
    printf("TachyonMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tachyon-miner");
    ...
    try { loop {
        ...
        printf("Running TachyonMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TachyonMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TachyonMiner, pwallet));
}
